...
function Menu({ children, items = [], hideOnClick = false, onChange = defaultFn, ...passProps }) {
    ...
    const renderItems = () => {
        return current.data.map((item, index) => {
            const isParent = !!item.children; // !! dùng để chuyển dữ liệu thành kiểu boolean, vì mình muốn cái nào có children thì trả về true, ko thì trả về false, trong khi children mình cần là 1 object
            return (
                <MenuItem
                    key={index}
                    data={item}
